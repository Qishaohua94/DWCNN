...
from models.DWCNN import DWCNNM, DWCNNS

parser = argparse.ArgumentParser()
parser.add_argument("-name", "--name", type=str, help="Name of the experiment", default="DWCNN")
...
parser.add_argument("-train_path", type=str, default="./modelnet40_20/*/train")
parser.add_argument("-val_path", type=str, default="./modelnet40_20/*/test")
...
if __name__ == '__main__':
    ...
    cnet = DWCNNS(args.name, nclasses=40, pretraining=pretraining, cnn_name=args.cnn_name)
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=130, shuffle=True, num_workers=6)

    val_dataset = SingleImgDataset(args.val_path, scale_aug=False, rot_aug=False, test_mode=True)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=130, shuffle=False, num_workers=6)
    print('num_train_files: '+str(len(train_dataset.filepaths)))
    print('num_val_files: '+str(len(val_dataset.filepaths)))
    trainer = ModelNetTrainer(cnet, train_loader, val_loader, optimizer, nn.CrossEntropyLoss(), 'dwcnn-s', log_dir, num_views=1)
    trainer.train(30)
    ...
    create_folder(log_dir)
    cnet_2 = DWCNNM(args.name, cnet, nclasses=40, cnn_name='resnet50', num_views=args.num_views)
    del cnet
    ...
    print('num_train_files: '+str(len(train_dataset.filepaths)))
    print('num_val_files: '+str(len(val_dataset.filepaths)))
    trainer = ModelNetTrainer(cnet_2, train_loader, val_loader, optimizer, nn.CrossEntropyLoss(), 'dwcnn-m', log_dir, num_views=args.num_views)
    trainer.train(50)
